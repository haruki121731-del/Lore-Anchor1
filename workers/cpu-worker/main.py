"""
lore-anchor CPU Worker Entrypoint

Pulls tasks from Redis queue (BLPOP on ``lore_anchor_tasks``), executes
the defense pipeline in CPU-only mode:
  Step 1: PixelSeal (DWT invisible watermark — no GPU required)
  Step 2: Mist v2 freq mode (NumPy DCT perturbation — no GPU required)
  Step 3: Verify watermark survived Mist
  Step 4: C2PA signing

Processing time: ~30–60s/image (GPU worker: ~5s).
This worker runs on Railway and handles Free plan users (max 5 images/month).
"""
...
# Mist v2 freq mode params (CPU-safe defaults — lighter than GPU VAE mode)
...
def _start_health_server() -> None:
    ...
    try:
        ...
        logger.warning(
            "Failed to start health check server on port %d",
            HEALTH_PORT,
            exc_info=True,
        )
...
# ---------------------------------------------------------------------------
# Pipeline (CPU-only mode)
# ---------------------------------------------------------------------------
def process_image(
    image_id: str,
    original_r2_key: str,
) -> dict[str, str | dict[str, Any] | None]:
    """Execute the CPU-only defense pipeline for a single image.

    Uses:
    - PixelSeal DWT backend (no GPU required)
    - Mist v2 ``freq`` mode (NumPy DCT — no VAE model download)
    - C2PA signing (CPU sufficient)
    ...
    """
    logger.info("Starting CPU pipeline for image_id=%s", image_id)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # --- Step: pixelseal (DWT — CPU only) ---
        try:
            logger.info("Step: pixelseal — embedding watermark (id=%s)", watermark_id)
            # device=None means CPU path in pixelseal
        ...
        # --- Step: mist_v2 (freq mode — NumPy DCT, no GPU) ---
        try:
            logger.info(
                "Step: mist_v2 — applying freq perturbation (epsilon=%d, steps=%d)",
                MIST_EPSILON,
                MIST_STEPS,
            )
            protected = apply_mist_v2(
                ...
                steps=MIST_STEPS,
                mode="freq",  # CPU-safe: NumPy DCT, no VAE model
            )
            protected.save(protected_path)
            logger.info("Step completed: mist_v2 (freq) for image_id=%s", image_id)
        except Exception as exc:
            raise PipelineStepError("mist_v2", exc) from exc

        # --- Step: verify_watermark ---
        try:
...
def _run_consumer() -> None:
    ...
    sb = _init_supabase()
    logger.info("CPU worker started, listening on queue: %s", QUEUE_KEY)
    ...
    while not _shutdown_requested:
        ...
        try:
            ...
            protected_r2_key: str = result_data["protected_r2_key"]  # type: ignore[assignment]
            protected_url: str = (
            ...
            try:
                _update_image_status(
                    ...
                    watermark_id=result_data["watermark_id"],  # type: ignore[arg-type]
                    c2pa_manifest=result_data.get("c2pa_manifest"),  # type: ignore[arg-type]
                )
            except Exception:
            ...
            _images_processed += 1
            logger.info("CPU pipeline completed: image_id=%s in %.1fs", image_id, elapsed)
...
if __name__ == "__main__":
    try:
        ...
        logger.info("=" * 60)
        logger.info("lore-anchor CPU Worker starting (freq mode, no GPU)")
        logger.info("=" * 60)
        logger.info("Redis URL: %s", REDIS_URL.split("@")[-1])
        logger.info("Mist config: epsilon=%d, steps=%d, mode=freq", MIST_EPSILON, MIST_STEPS)
